...
void initTimer0_Periodic(uint32_t mdelay);
void initTimer0_Periodic_Pres(uint32_t sec);
...
void initTimer0_Periodic(uint32_t mdelay){
	...
	TIMER0->TAILR = clock1ms - 1;		// Establecer el rango de tiempo
	TIMER0->ICR = 0x01;
	TIMER0->CTL |= 0x01;
	for(uint32_t i=0; i < mdelay; i++){
...
void initTimer0_Periodic_Pres(uint32_t sec){
	...
	TIMER0->TAMR = 0x02;		// periodic
	TIMER0->TAILR = 62500 - 1;		// Establecer el rango de tiempo
	TIMER0->TAPR = 256-1;
	TIMER0->ICR = 0x01;
	TIMER0->CTL |= 0x01;
